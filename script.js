...
document.addEventListener("DOMContentLoaded", function(e) {
	...
	$(document).on('change keyup paste', '#input, #code', function(e) {
		if( $('#live_mode').is(':checked') )
		{
			if( $(this).attr('id')=='input' && $('#input').val().trim()!='' )
				$('#code').val( $('#input').val().trim().hexEncode($('#space').is(':checked'), $('#prepend').is(':checked')));
			else if( $('#code').val().trim()!='' )
				$('#input').val( $('#code').val().trim().hexDecode() );
		}
	});
